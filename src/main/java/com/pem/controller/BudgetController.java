...
public class BudgetController {
	...
	public String viewBudgets(HttpSession session, Model model) {
		String email = (String) session.getAttribute("email");
		if (email == null) {
			model.addAttribute("errMsg", "Session expired. Please login again.");
			return "login";
		...
		List<BudgetResponseDto> budgets = budgetService.getBudgetsByUserId(email);
		model.addAttribute("budgets", budgets);
		return "Budget/budget_list"; // Create this JSP
	}
